...
from pytorch_pretrained_bert.modeling import BertForSequenceClassification, BertForNextSentencePrediction
...
class MrpcProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {}".format(os.path.join(data_dir, "train_p.tsv")))
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train_p.tsv")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "dev_p.tsv")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "eval2_unlabelled_p.tsv")), "eval2_unlabelled")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_b = line[2]  # answer
            label = '1'  # useles label harcoding
            # print(label)
            examples.append(
...
def main():
    ...
    parser.add_argument("--data_dir",
                        ...
                        type=str,
                        required=False,
                        help="The input data dir. Should contain the _p.tsv files (or other data files) for the task.")
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        data = pd.read_csv('../data/bert/eval2_unlabelled_p.tsv', sep='\t', header=None)
        data[len(data.columns)] = predictions
        data.to_csv('../data/pre-answer.tsv', sep='\t', header=False, index=False)

        # Converting to submission format
        data = pd.read_csv('../data/pre-answer.tsv', sep='\t', names=['query_id', 'query_text', 'passage_text', 'passage_id', 'cs'])
